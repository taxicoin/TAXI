...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Taxi: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Taxi Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    {
        // Subsidy is cut in half every 512,640 blocks, which will occur approximately every 2 years
        nSubsidy >>= (nHeight / 512640); // Taxi: 512k blocks in ~2 years
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Taxi: every 24 hours
static const int64 nTargetSpacing = 2 * 60; // Taxi: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // Taxi: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal taxi miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Taxi", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "11 January 2014, Almost 500 reported killed in Syria";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1389402961;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 108768528;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Taxi: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Taxid: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
